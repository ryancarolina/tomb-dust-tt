...
from tomb_gm.services.rag import index_rules, search_rules


def register(sub: argparse._SubParsersAction) -> None:
    rules = sub.add_parser("rules", help="Search build/systems rules canon")
    rules_sub = rules.add_subparsers(dest="rules_cmd", required=True)

    p_search = rules_sub.add_parser("search", help="Search rules by keyword")
    ...
    p_index = rules_sub.add_parser("index", help="Rebuild rules search index")
...
def handle_index(args: argparse.Namespace, _ctx_unused: CommandContext | None = None) -> dict:
    ctx = _ctx(args)
    stats = index_rules(ctx.conn, ctx.config.content_root)
